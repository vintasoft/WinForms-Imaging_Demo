...
namespace ImagingDemo
{
    internal class ImageProcessingCommandExecutor
    {
        ...
        /// Executes the IsImageBlankCommand command.
        /// </summary>
        public void ExecuteIsInvertedCommand()
        {
#if !REMOVE_DOCCLEANUP_PLUGIN
            IsDocumentImageInvertedCommand command = new IsDocumentImageInvertedCommand();
            ...
            if (ExecuteProcessingCommand(command, false))
            {
                ...
                {
                    if (command.Result)
                        MessageBox.Show("Document image is inverted.");
                    else
                        MessageBox.Show("Document image is not inverted.");
                }
                else
                {
        ...
        private bool CommandCanProcessImage(ProcessingCommandBase command, VintasoftImage image)
        {
            ...
            if (command.IsPixelFormatSupported(pixelFormat))
                return true;
